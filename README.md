...
To create the log file, edit the start_headless_server.bat in the server directory to include -logfile "[desired location]" in the launch arguments.
...
In the server monitor GUI, you can type the absolute path to the log file (e.g. C:/some_folder/log.txt) and click the "Load Log" button to begin monitoring. If the server is shut down and restarted while the monitor is still running, you must click "Load Log" again to begin monitoring the new log.
